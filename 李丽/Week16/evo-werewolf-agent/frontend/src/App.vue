...
function processStepResponse(resp) {
  ...
  // Game over detection and summary generation
  if (resp.is_game_over) {
    if (resp.phase === 'summary' || resp.phase === 'game_over') {
      // Summaries are actually ready
      ...
      // Game over just triggered, we need one more step to generate summaries.
      // If we are auto-playing, it will naturally fetch the next step.
      // If manual, the user can click the "单步执行" button because isGameOver is still false.
      stepLog.value.push({
        id: Date.now() + Math.random(),
        type: 'gameover',
        text: `🏆 游戏结束！即将生成玩家总结...`,
