...
"""
    ...
    def _load_evolved_prompt(self) -> str:
        """加载演化后的提示词
        
        从 agent/prompts/{role_type}.txt 加载优化后的提示词。
        如果文件不存在，返回空字符串。
        """
        if not self.role_type:
            return ""
        
        prompt_path = os.path.join(os.path.dirname(__file__), "prompts", f"{self.role_type}.txt")
